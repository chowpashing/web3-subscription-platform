...
async function main() {
  console.log("Starting NewBotPayment gas consumption test...");
  ...
  const botPayment = await NewBotPaymentFactory.deploy(
    await usdt.getAddress(),
    await botRegistry.getAddress(),
    await botSubscription.getAddress()
  ) as unknown as NewBotPayment;
  await botPayment.waitForDeployment();
  console.log("NewBotPayment deployed to:", await botPayment.getAddress());
  ...
  // Test NewBotPayment gas consumption
  console.log("\n3. Testing NewBotPayment gas consumption...");
